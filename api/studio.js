  ...
  try {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const studioImage = ogImages.studio?.stringValue;

        if (studioImage) {
          ogImageUrl = studioImage;
          console.log('Using og:image from Firestore (studio):', ogImageUrl);
        }
      }
    } catch (fetchError) {
    ...
    console.error('Error generating studio page:', error);
    try {
      const htmlPath = path.join(process.cwd(), 'public', 'studio.html');
      ...
      return res.status(500).json({ error: 'Failed to serve studio page' });
    }
  }
};
