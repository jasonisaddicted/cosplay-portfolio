...
module.exports = async (req, res) => {
  ...
  try {
    ...
    let ogImageUrl = 'https://cosplay-portfolio.vercel.app/og/collaborators-cyX7hTzl288bb468cDVt.jpg';
    ...
    try {
      ...
      if (response.ok) {
        ...
        const collabsImage = ogImages.collabs?.stringValue;

        if (collabsImage) {
          ogImageUrl = collabsImage;
          console.log('Using og:image from Firestore (collabs):', ogImageUrl);
        }
      }
    } catch (fetchError) {
    ...
    const htmlPath = path.join(process.cwd(), 'public', 'collabs.html');
    ...
    console.error('Error generating collabs page:', error);
    try {
      const htmlPath = path.join(process.cwd(), 'public', 'collabs.html');
      ...
      return res.status(500).json({ error: 'Failed to serve collabs page' });
    }
  }
};
